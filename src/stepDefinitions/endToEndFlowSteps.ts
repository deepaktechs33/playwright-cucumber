...
When(
  'the user enters {string} {string} {string} as checkout details',
...
Then('the user should be navigated to the checkout overview page', async function (this: CustomWorld) {
  expect(await this.checkoutOverviewPage.getPageTitle(), 'Checkout overview page did not load as expected').toEqual(
    'Checkout: Overview'
  );
});
...
Then('the user should be back on the products page', async function (this: CustomWorld) {
  expect(await this.homePage.getPageTitle(), 'User was not navigated back to the products page').toEqual('Products');
});
